...
IOManager::IOManager(const ConfigManager & config_manager) :
    IOManager(config_manager.getSaveMode(),
              ...
              config_manager.getSaveInterval(),
              config_manager.getSaveQuality()) {}
...
IOManager::IOManager(std::string save_mode,
    ...
    save_quality_(save_quality) {
    // 一次性探测 CUDA 可用性，避免 readNextFrame 每帧 cudaGetDeviceCount
    int device_count = 0;
    has_cuda_ = (cudaGetDeviceCount(&device_count) == cudaSuccess && device_count > 0);
}
...
void IOManager::saveFrame(const cv::Mat & frame, int num_frames) {
    // 图片模式：按 save_interval 帧间隔落盘，避免 JPEG 编码/写盘拖慢主循环
    bool save_this = (save_interval_ <= 1) || (num_frames % save_interval_ == 0);
    if ((save_mode_ == "images" || save_mode_ == "both") && save_this) {
        std::string          save_path = out_dir_ + "/frame_" + std::to_string(num_frames) + ".jpg";
        std::vector<int>     jpeg_params = { cv::IMWRITE_JPEG_QUALITY, save_quality_ };
        cv::imwrite(save_path, frame, jpeg_params);
    }
...
bool IOManager::openVideoSource(const std::string & video_path) {
    // 纯数字当作摄像头设备号，否则当文件路径
    if (!video_path.empty() && std::all_of(video_path.begin(), video_path.end(), ::isdigit)) {
        video_capture_.open(std::stoi(video_path), cv::CAP_V4L2);
    } else {
