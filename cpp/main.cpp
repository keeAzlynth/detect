...
[[nodiscard]] cv::Mat drawOneFrame(FrameInputContext &            frame_input_context,
                     ...
                     int                            total_us) {
    if (infer_output_context.tracked_objects.empty()) {
...
int run(char * video_path, char * config_path) {
    ...
    // 异步流水线模式：读帧/推理/绘图重叠执行
    AsyncPipeline async_pipeline(config_manager, frame_meta, 2);
    async_pipeline.start(io_manager);
    
    while (true) {
        // 从异步队列获取帧
        auto frame_ctx = async_pipeline.getNextFrame();
        if (!frame_ctx) {
            break;
        }
        // 复制帧数据到主循环的 frame_input_context
        frame_input_context.frame_id = frame_ctx->frame_id;
        frame_input_context.raw_img = frame_ctx->raw_img;
        frame_input_context.d_raw_img_ = std::move(frame_ctx->d_raw_img_);
        frame_input_context.timestamp = frame_ctx->timestamp;
        num_frames = frame_input_context.frame_id;
